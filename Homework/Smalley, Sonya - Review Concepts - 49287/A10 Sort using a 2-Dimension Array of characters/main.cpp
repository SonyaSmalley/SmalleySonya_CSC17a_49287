/* 
 * Note:  This time you create the strcmp() function that takes into
 * account the new sort order.
 */

//System Libraries Here
#include <iostream>//cin,cout
#include <cstring> //strlen(),strcmp(),strcpy()
using namespace std;

//Global Constants Only, No Global Variables
//Allowed like PI, e, Gravity, conversions, array dimensions necessary
const int COLMAX=80;//Only 20 required, and 1 for null terminator

//Function Prototypes Here
int  read(char [][COLMAX],int &);//Outputs row and columns detected from input
void sort(char [][COLMAX],int,int,const char[],const char[]);//Sort by row using strcmp();
void print(const char [][COLMAX],int,int);//Print the sorted 2-D array
//int strcmp(char a[],char b[],char replace[],char with[]){
int strcmp(char [],char [],const char [],const char []);//Replace sort order

//Program Execution Begins Here
int main(int argc, char** argv) {
    //Declare all Variables Here
    const int ROW=30;             //Only 20 required
    char array[ROW][COLMAX];      //Bigger than necessary 
    int colIn,colDet,rowIn,rowDet;//Row, Col input and detected
    char replace[COLMAX],with[COLMAX];//New sort order
    
    //Input the new sort order then sort
    cout<<"Modify the sort order by swapping these characters."<<endl;
    cin>>replace;
    cout<<"With the following characters"<<endl;
    cin>>with;
    cout<<"Read in a 2 dimensional array of characters and sort by Row"<<endl;
    cout<<"Input the number of rows <= 20"<<endl;
    cin>>rowIn;
    cout<<"Input the maximum number of columns <=20"<<endl;
    cin>>colIn;
    
    //Now read in the array of characters and determine it's size
    rowDet=rowIn;
    cout<<"Now input the array."<<endl;
    colDet=read(array,rowDet);
    
    //Compare the size input vs. size detected and sort if same
    //Else output different size
    //if(rowDet==rowIn&&colDet==colIn){
        sort(array,rowIn,colIn,replace,with);
        cout<<"The Sorted Array"<<endl;
        print(array,rowIn,colIn);
    
    //Exit
    return 0;
}

int  read(char a[][COLMAX],int &maxRow)//Outputs row and columns detected from input
{
    int row=0,col=0,maxCol=0;
    
    cin.ignore();
    while (cin.getline(a[row],COLMAX))
    {
        row++;
    }
    
    for (int i=0;i<row;i++)
    {    
        col=strlen(a[i]);
        if (col>=maxCol&&col<COLMAX)
            maxCol=col;
    }
    
    if (row!=maxRow)
        maxRow=row;
    
    return maxCol;
}

void sort(char a[][COLMAX],int row,int col,const char replace[],const char with[])//Sort by row using strcmp()
{
    for (int last=row-1;last>0;last--)
    {
        for (int r=0;r<last;r++)    //compare two rows
        {
            if (strcmp(a[r],a[r+1],replace,with)>0) //compare the rows for sorting according to the new world order
            {
                char temp[col];
                strcpy (temp,a[r]); //switch the values in the rows
                strcpy (a[r],a[r+1]);
                strcpy (a[r+1],temp);
            }
        }
    }
}

void print(const char a[][COLMAX],int row,int col)//Print the sorted 2-D array
{
    for (int r=0;r<row;r++) //for each row
    {
        for (int c=0;c<col;c++) //print each element column by column
            cout<<a[r][c];  //print
        cout<<endl;
    }
}

//int strcmp(char a[],char b[],char replace[],char with[])
int strcmp(char a[],char b[],const char replace[],const char with[])//Replace sort order
{
    int repLen,aLen,r=0;
    bool aDone=0,bDone=0;
    
    repLen=strlen(replace); //get the length of the replace and with arrays
    aLen=strlen(a); //get the length of the a array
    
    char a2[aLen],b2[aLen];
    
    strcpy(a2,a);
    strcpy(b2,b);
    
    while (a2[r]!='\0')
    {
        for (int i=0;i<repLen;i++)
        {   
            if (a2[r]==replace[i]&&aDone==0)
            {
                a2[r]=with[i];
                aDone=1;
            }
            if (b2[r]==replace[i]&&bDone==0)
            {
                b2[r]=with[i];
                bDone=1;
            }
        }
        r++;
    }
    
    r=0; 
    do
    {
        if (a2[r]>b2[r])
            return 1;
        if (a2[r]<b2[r])
            return -1;
        if (b2[r]=='\0')
            return 1;
        r++;
    }
    while (a2[r]!='\0');
    if (b2[r]!='\0')
        return -1;
    
    return 0;
}