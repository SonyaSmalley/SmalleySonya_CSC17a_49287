...
int *mode(const int *array,int arySize){
    ...
    int count=0, //Starting frequency for any value in the array is 1
        maxCnt=1, //Highest frequency starts at 1
        curVal=*ary; //The current value being counted - starts with the first value in the array
    for (int i=0;i<arySize;i++)
    {
        if (curVal==*(ary+i)) //If the current element also holds the value we're currently counting...
            count++;
        else
        {
            if (count>maxCnt) //If the total number counted for the current value is greatest
                maxCnt=count;
            count=1;
        }
        curVal=*(ary+i); //The current array element value will be compared to the next element
    }
    
    //Find the number of modes
    int nModes=0; //The number of modes in the array starts at 0
    count=0; //Starting frequency for any value in the array is 1, but you start at 0 at the very beginning
    curVal=*ary; //The current value being counted - starts with the first value in the array
    if (maxCnt>1)
    {
        for (int i=0;i<arySize;i++)
        {
            ...
            {
                if (count==maxCnt) //If the total number counted for the current value is equal to the highest frequency of numbers
                    nModes++;
                count=1; //Once you get started, there will never be less of any number you count
            }
            curVal=*(ary+i); //The current array element value will be compared to the next element
        }
    }
    ...
    modeAry[0]=nModes; //The number of modes in the array
    modeAry[1]=maxCnt; //The max frequency of any number in the array
    
    //Fill the mode array
    int indx=2; //Use to fill modeAry - it will track which modeAry elements are filled
    for (int i=0;i<arySize;i++)
    {
        if (curVal==*(ary+i)) //If the current element also holds the value we're currently counting...
            count++;
